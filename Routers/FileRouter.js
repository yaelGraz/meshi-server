...
// נשתמש ב-memoryStorage כי אנחנו מעלים ישר ל-Supabase
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 100 * 1024 * 1024 } });

// רשימת קבצים
// GET /files/:category/:subcategory
 FileRouter.get("/:category/:subcategory", FileController.list);

// תוכן קובץ לצפייה/הורדה
// GET /files/:category/:subcategory/fileContent/:guidName
 FileRouter.get("/:category/:subcategory/fileContent/:guidName", FileController.fileContent);

// העלאה (multipart/form-data, field: "file")
// POST /files/upload?category=...&subcategory=...
 FileRouter.post("/upload", upload.single("file"), FileController.upload);

// החלפת קובץ קיים לפי guidName (אותו נתיב בבאקט)
